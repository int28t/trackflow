...
func TestParseBatchValidation(t *testing.T) {
	t.Parallel()

	if _, err := parseBatch("bad"); err == nil {
		t.Fatal("expected parse error for non-integer batch")
	}

	if _, err := parseBatch("0"); err == nil {
		t.Fatal("expected parse error for zero batch")
	}

	batch, err := parseBatch("10")
	if err != nil {
		t.Fatalf("parseBatch returned error: %v", err)
	}

	if batch != 10 {
		t.Fatalf("unexpected parsed batch: got %d, want %d", batch, 10)
	}
}
