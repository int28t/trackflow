...
func TestMiddlewareAndHandlerCollectMetrics(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/notifications/send", nil)
...
func TestRoutePatternAndHelpers(t *testing.T) {
	...
	req.Pattern = "/internal/notifications/send"
	if got := routePattern(req); got != "/internal/notifications/send" {
